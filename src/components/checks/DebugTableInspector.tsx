// '@/components/checks/DebugTableInspector'
...
export function DebugTableInspector() {
  ...
  if (!db || !db.schema) {
    return <Text style={styles.emptyText}>Database initializing...</Text>;
  }

  // Get table names dynamically from the db schema
  const tableNames = Object.keys(db.schema.tables);
  ...
  const refreshData = async () => {
    ...
    try {
      const records = await db.get(selectedTable).query().fetch();
