// @kuvali-js/core/i18n/lib/i18n.ts
  ...
  function t<S extends DotPathsFor, A extends Params<S>>(key: S, args?: A) {
    for (const locale of orderedLocales) {
      const translationObject =
        translations[locale.toLowerCase() as Lowercase<string>]; // translations of one language
      ...
      if (translationObject !== null) {
        if (translationObject["markMissingTranslation"] !== undefined) {
          ...
          return (
            translationObject.i18n_meta.markMissingTranslation as string
          ).replace("{key}", key);
        }
      }
 ...
 */
const inMap = (map: object, key: string) =>
  Object.keys(map as object).includes(key);
    ...
    switch (argTypeTrim) {
      ...
      case "plural": {
        if (typeof argValue === "string")
          argValue = Number.parseFloat(argValue);
        ...
        switch (true) {
          ...
          case argValue >= 0 &&
            argValue <= 10 &&
            inMap(pluralMap as object, "format"):
            if (pluralMap?.format === "strings") {
              replacement = replacement!.replace(
                `{?}`,
                pluralMap?.strings[argValue],
              );
              break;
            }
        }
      ...
      default: {
        if (typeof argValue !== "string")
          throw new Error("i18n: Invalid argument");
        ...
        if (argTypeTrim != undefined) {
          ...
          replacement = getTranslation(locale, translations, argTypeTrim, {
            [argKey]: argValue,
          });
          if (replacement == undefined)
            throw new Error(
              `i18n.ts >case: default<:\nTranslation: Missing replacement value:\nkey ${argKey}\nnot found in literals, all translation or provided a value.`,
            );
        } else {
